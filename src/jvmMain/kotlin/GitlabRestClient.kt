...
class GitlabRestClient(
    ...
    val gitlabServiceRootUrl = "${rootUrl}/api/v4"

    @Suppress("LeakingThis")
    val gitlabOAuthUrl = "${rootUrl}/oauth/"
    ...
    fun restTemplate(builder: RestTemplateBuilder): RestTemplate = builder.build().apply {
    ...
    fun getBranch(token: String, projectId: Long, branch: String): Branch {
        return GitlabHttpEntity("", createUserHeaders(token))
            .addErrorDescription(
                404,
                ErrorCode.GitlabBranchNotExists,
                "Cannot get branch $branch in project with id $projectId."
            )
            .makeRequest {
    ...
    fun getBranches(token: String, projectId: Long): List<Branch> {
        return GitlabHttpEntity("", createUserHeaders(token))
            .addErrorDescription(
                404,
                ErrorCode.GitlabProjectNotExists,
                "Cannot get branches of project with id $projectId."
            )
            .makeRequest {
                val url = "$gitlabServiceRootUrl/projects/$projectId/repository/branches"
    ...
    fun deleteBranch(token: String, projectId: Long, targetBranch: String) {
        GitlabHttpEntity(null, createUserHeaders(token))
            .addErrorDescription(
                ErrorCode.GitlabBranchDeletionFailed,
                "Cannot delete branch $targetBranch in project with id $projectId"
            )
            .makeRequestNoBody {
    ...
    fun adminGetUserById(id: Long): GitlabUser {
        return GitlabHttpEntity<String>("body", createAdminHeaders())
            .addErrorDescription(
                404,
                ErrorCode.GitlabUserNotExisting,
                "Cannot find user by id as admin. User does not exist"
            )
            .addErrorDescription(ErrorCode.GitlabUserNotExisting, "Cannot find user by token as admin")
            .makeRequest {
    ...
    fun assertConnection(): String? {
        log.info("HEALTH-CHECK: GITLAB_ROOT_URL is set to ${rootUrl.censor()}")
        log.info("HEALTH-CHECK: GITLAB_ADMIN_TOKEN is set to ${adminUserToken.censor()}")
        if (rootUrl.isBlank()) {
            throw Error("FATAL: GITLAB_ROOT_URL is empty: ${rootUrl}")
        }
        if (adminUserToken.isBlank()) {
            throw Error("FATAL: GITLAB_ADMIN_TOKEN is empty: $adminUserToken")
        }
        try {
            ...
            if (e.statusCode == 403) {
                throw Error(
                    "FATAL: Provided GITLAB_ADMIN_TOKEN is not allowed: ${adminUserToken.censor()}",
                    e
                )
            }
    ...
    private fun forceRefreshAdminOAuthToken(): String {
        synchronized(this) {
            this.userLoginOAuthToGitlab(userName = adminUsername, password = adminPassword)
